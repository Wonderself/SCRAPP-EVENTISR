...
class MatkonetScraper(BaseScraper):
    """Matkonet event scraper"""
    
    def __init__(self):
        super().__init__(EventSource.MATKONET)
    
    async def scrape_events(self) -> List[Dict]:
        """Scrape events from Matkonet"""
        events = []
        
        try:
            # Navigate to Matkonet events page
            if not await self.navigate_to_events_page():
                return events
            
            # Handle Hebrew site navigation
            ...
            while page_count < max_pages:
                logger.info(f"Scraping Matkonet page {page_count + 1}")
            ...
            logger.info(f"Found {len(events)} events on Matkonet")
            return events
            
        except Exception as e:
            error_msg = f"Error scraping Matkonet: {e}"
            logger.error(error_msg)
            self.errors.append(error_msg)
            return events
    
    async def navigate_to_events_section(self):
        """Navigate to events section on Matkonet"""
        try:
    ...
    async def apply_date_filter(self):
        ...
        try:
            # Look for date filter elements (Hebrew site)
            date_selectors = [
                '[data-filter="date"]',
                '.date-filter',
                'select[name*="date"]',
    ...
    async def extract_events_from_page(self) -> List[Dict]:
        ...
        try:
            # Get all event cards
            event_cards = await self.page.query_selector_all('.event-item')
            
            if not event_cards:
                # Try alternative selectors for Hebrew site
    ...
    async def extract_event_basic_info(self, event_element) -> Optional[Dict]:
        """Extract basic event information from Matkonet event card"""
        try:
            # Matkonet-specific selectors (Hebrew site)
            selectors = {
                'title': ['.event-title', '.title', 'h3', 'h2', '.כותרת'],
                'date': ['.event-date', '.date', '.תאריך', '.event-time'],
                'location': ['.event-location', '.location', '.מיקום', '.venue'],
                'url': ['a[href]'],
                'description': ['.event-description', '.description', '.תיאור', '.summary'],
                'price': ['.event-price', '.price', '.מחיר', '.ticket-price']
            }
            ...
            url = ScrapingUtils.normalize_url(url, "https://www.matkonet.co.il")
