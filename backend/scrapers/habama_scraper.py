...
class HabamaScraper(BaseScraper):
    """Habama event scraper"""
    
    def __init__(self):
        super().__init__(EventSource.HABAMA)
    
    async def scrape_events(self) -> List[Dict]:
        """Scrape events from Habama"""
        events = []
        
        try:
            # Navigate to Habama events page
            ...
            while page_count < max_pages:
                logger.info(f"Scraping Habama page {page_count + 1}")
            ...
            logger.info(f"Found {len(events)} events on Habama")
            return events
            
        except Exception as e:
            error_msg = f"Error scraping Habama: {e}"
            logger.error(error_msg)
            self.errors.append(error_msg)
            return events
    
    async def navigate_to_events_section(self):
        """Navigate to events section on Habama"""
        try:
            # Look for events navigation links (Hebrew theater site)
            events_links = [
                'a[href*="show"]',
                'a[href*="הצגה"]',
                'text="הצגות"',
    ...
    async def apply_date_filter(self):
        ...
        try:
            # Look for date filter elements (Hebrew theater site)
            date_selectors = [
                '[data-filter="date"]',
                '.date-filter',
                'select[name*="date"]',
    ...
    async def extract_events_from_page(self) -> List[Dict]:
        ...
        try:
            # Get all event cards
            event_cards = await self.page.query_selector_all('.show-item')
            
            if not event_cards:
                # Try alternative selectors for theater site
    ...
    async def extract_event_basic_info(self, event_element) -> Optional[Dict]:
        """Extract basic event information from Habama event card"""
        try:
            # Habama-specific selectors (Hebrew theater site)
            selectors = {
                'title': ['.show-title', '.title', 'h3', 'h2', '.name', '.כותרת'],
                'date': ['.show-date', '.date', '.time', '.when', '.תאריך'],
                'location': ['.show-venue', '.venue', '.location', '.where', '.מקום'],
                'url': ['a[href]'],
                'description': ['.show-description', '.description', '.summary', '.תיאור'],
                'price': ['.show-price', '.price', '.cost', '.ticket-price']
            }
            ...
            url = ScrapingUtils.normalize_url(url, "https://www.habama.co.il")
